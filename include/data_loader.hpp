#pragma once
/**
 * From: https://github.com/yaroslavbohonos/Handwritten-Digit-Recognition/blob/main/src/data_loader.h
 */
#include <cstdio>
#include <cstdint>
...
public:
    ...
    bool Load (bool training)
    {
        ...
        std::filesystem::path file_path("../data/Data");
        FILE* file = fopen((file_path / labelsFileName).c_str(),"rb");
        if (!file)
        {
            printf("Could not open %s for reading.\n", (file_path / labelsFileName).c_str());
            return false;
        }
        ...
        file = fopen((file_path / imagesFileName).c_str(), "rb");
        if (!file)
        {
            printf("Could not open %s for reading.\n", (file_path / imagesFileName).c_str());
            return false;
        }
        fseek(file, 0, SEEK_END);
        fileSize = ftell(file);
